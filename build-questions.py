#!/usr/bin/env python3
"""Parse the SecExams CAD PDF text into structured JSON of questions.

The CAD (ServiceNow Certified Application Developer) dump is the same SecExams
format, containing single- and multi-answer questions (no drag-and-drop).
"""
import json
import re
import subprocess
...
txt = subprocess.run(
    ["pdftotext", "cad.pdf", "-"], capture_output=True, text=True
).stdout
...
noise_patterns = [
    ...
    re.compile(r"^\s*ServiceNow.* - CAD Practice Questions - SecExams\.com\s*$"),
